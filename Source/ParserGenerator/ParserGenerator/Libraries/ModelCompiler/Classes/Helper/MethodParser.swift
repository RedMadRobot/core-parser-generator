...
open class MethodParser: KlassContentParser<Method> {
    ...
    open override func appendContent(line: SourceCodeLine)
    {
        ...
        if !self.isBlockCommentScope {
            let currentLine: String = cleanLine.truncateToWord(")").truncateLeadingWhitespace()
    ...
    open override func endedParsingContent(line: SourceCodeLine, documentation: [SourceCodeLine]) throws -> Method?
    {
        ...
        if self.declaration!.line.contains(" init(") {
            ...
            name = self.declaration!.line.truncateToWord(" func ", deleteWord: true).truncateFromWord("(")
            returnType = try self.findReturnType(declaration: self.returnType)
        }
        
        arguments = try ArgumentParser().parse(argumentDeclarations: self.arguments)
...
private extension MethodParser {
    ...
    func findReturnType(declaration: SourceCodeLine?) throws -> Typê?
    {
        ...
        let rawType: String = typeDeclaration.line.truncateToWord("->", deleteWord: true).truncateFromWord("{").trimmingCharacters(in: CharacterSet.whitespaces)
        ...
        return try TypeParser().parse(rawPropertyLine: rawTypeDeclaration)
    }
